...
const ClimatScreen = () => {
  ...
  useEffect(() => {
    axios
      .get(`https://api.weatherapi.com/v1/forecast.json?key=${apiKey}&q=lyon&days=3&lang=fr`)
      .then((res) => {
        ...
        const tomorrowHours = remainingHours > 0 
          ? res.data.forecast.forecastday[1].hour.slice(0, remainingHours)
          : [];
  ...
  if (!forecast || hourlyData.length === 0) return <Text style={styles.loading}>Chargement...</Text>;

  const tempsData = hourlyData.map(hour => hour.temp_c);
  const timeLabels = hourlyData.map(hour => 
    new Date(hour.time).getHours() + 'h'
  );

  // Composant pour les points interactifs
  ...
  // Composant pour la ligne de sélection
  const Tooltip = ({ x, y }) => {
    ...
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.todayCard}>
        ...
        <Image
          ...
          style={styles.icon}
        />
        <Text style={styles.condition}>{forecast[0].day.condition.text}</Text>
        <Text style={styles.temp}>
          {forecast[0].day.mintemp_c}°C - {forecast[0].day.maxtemp_c}°C
        </Text>
        <Text style={styles.info}>Humidité: {forecast[0].day.avghumidity}%</Text>
        <Text style={styles.info}>Vent: {forecast[0].day.maxwind_kph} km/h</Text>
      </View>
...
labelContainer: {
  ...
  width: 30, // assure l'espace pour que chaque label tienne sur une ligne
},
